...
export default function UserDashboard() {
  ...
  const { resumes, setActiveResumeId, createNewResume, deleteResume, isLoading, isSaving, error, user, logout } = useApp();
  ...
  const statsCards = [
    ...
    { label: 'Avg ATS Score', value: resumes.length ? `${Math.round(resumes.reduce((s, r) => s + r.atsScore, 0) / resumes.length)}%` : '0%', icon: 'analytics', color: 'text-emerald-600 bg-emerald-50' },
  ...
  return (
    <div className="flex min-h-screen text-left font-sans bg-surface">
      ...
      <aside className="hidden md:flex h-screen w-60 fixed left-0 top-0 bg-surface-container-lowest border-r border-outline-variant/20 flex-col p-4 gap-2 shadow-sm z-30">
        ...
        {/* User */}
        <div className="relative border-t border-outline-variant/20 pt-3">
          <button
            onClick={() => setProfileOpen((open) => !open)}
            className="flex w-full items-center gap-3 rounded-xl px-2 py-2 text-left transition-all hover:bg-surface-container-low"
            title="Account menu"
          >
            <span className="w-9 h-9 rounded-full bg-primary text-on-primary overflow-hidden flex items-center justify-center text-xs font-extrabold shrink-0">
              {initials}
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-bold text-on-surface leading-none truncate">{displayName}</span>
              <span className="block text-[10px] text-on-surface-variant truncate mt-1">{displayEmail}</span>
            </span>
            <span className="material-symbols-outlined text-[18px] text-on-surface-variant">expand_more</span>
          </button>

          {profileOpen && (
            <div className="absolute bottom-16 left-0 right-0 rounded-2xl border border-outline-variant/30 bg-white p-2 shadow-2xl">
              <button
                onClick={() => { setProfileOpen(false); navigate('/dashboard'); }}
                className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-on-surface-variant hover:bg-surface-container-low hover:text-primary"
              >
                <span className="material-symbols-outlined text-[20px]">dashboard</span>
                Dashboard
              </button>
              <button
                ...
                className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-on-surface-variant hover:bg-red-50 hover:text-red-600"
              >
        ...
        <header className="sticky top-0 z-20 backdrop-blur-xl bg-surface/90 border-b border-outline-variant/30 px-6 md:px-8 py-3.5 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-3">
            ...
            {(isLoading || isSaving || error) && (
              <span className={`hidden sm:inline-flex items-center gap-1 rounded-full px-3 py-1 text-[11px] font-bold ${
                error ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-primary/5 text-primary'
              }`}>
                <span className="material-symbols-outlined text-[14px]">{error ? 'cloud_off' : isLoading ? 'sync' : 'cloud_done'}</span>
                {error ? 'Local demo mode' : isLoading ? 'Loading' : 'Saved'}
              </span>
            )}
          </div>
          <div className="flex items-center gap-3">
            ...
            <section className="lg:col-span-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute top-4 right-4 p-1 text-on-surface-variant hover:text-red-600 transition-colors rounded-lg hover:bg-red-50"
                        title="Delete resume"
                      >
                ...
                {sortedResumes.length === 0 && !isLoading && (
                  <div className="md:col-span-2 bg-white rounded-2xl border border-outline-variant/20 p-8 text-center shadow-sm">
                    <span className="material-symbols-outlined text-primary text-[42px] mb-3">description</span>
                    <h4 className="text-lg font-bold text-on-surface mb-1">No resumes yet</h4>
                    <p className="text-sm text-on-surface-variant mb-5">Create your first resume and ResumeIQ will open the editor for you.</p>
                    <button
                      onClick={handleCreateNew}
                      className="inline-flex items-center gap-2 bg-primary text-on-primary px-5 py-2.5 rounded-full text-sm font-bold hover:shadow-md active:scale-95 transition-all"
                    >
                      <span className="material-symbols-outlined text-[18px]">add</span>
                      Create Resume
                    </button>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl border border-outline-variant/20">
            <div className="flex items-start gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center">
                <span className="material-symbols-outlined">delete</span>
              </div>
              <div>
                <h4 className="font-bold text-on-surface">Delete this resume?</h4>
                <p className="text-sm text-on-surface-variant mt-1">This removes it from your workspace and cannot be undone.</p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setDeleteId(null)}
                className="px-4 py-2 rounded-xl border border-outline-variant text-sm font-bold text-on-surface-variant hover:bg-surface-container-low"
              >
                Cancel
              </button>
              <button
